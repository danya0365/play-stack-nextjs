...
export function LessonView({
  ...
  const [activeTab, setActiveTab] = useState<"content" | "code" | "challenge">(
    "content"
  );
  ...
  const markComplete = () => {
    setIsCompleted(true);
    // In production, call API to save progress
  };
    ...
    return (
      <div className="retro-page h-full overflow-auto">
        ...
        <div className="retro-groupbox flex-1 overflow-auto">
            ...
            <div className="space-y-4 p-2">
              {lesson.codeExamples.map((ex, i) => (
                <div key={i}>
                  ...
                  <pre className="bg-black text-green-400 p-2 text-xs overflow-auto">
                    {ex.code}
                  </pre>
                </div>
              ))}
          ...
          {activeTab === "challenge" && lesson.challenge && (
            <div className="p-2 text-xs">
              ...
              <pre className="bg-black text-green-400 p-2 overflow-auto">
        ...
        <div className="flex justify-between mt-2">
          {prevLesson ? (
            <Link
              href={`/courses/${phaseId}/${moduleId}/${prevLesson.id}`}
          ...
          {!isCompleted && isLoggedIn && (
            <button onClick={markComplete} className="retro-btn retro-btn-primary text-xs">
              ✅ เสร็จสิ้น
            </button>
          )}
          {nextLesson ? (
            <Link
              href={`/courses/${phaseId}/${moduleId}/${nextLesson.id}`}
              className="retro-btn text-xs"
            >
              ถัดไป →
            </Link>
    ...
    <div className="h-full overflow-auto">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {!isCompleted && isLoggedIn && (
              <button onClick={markComplete} className="main-btn main-btn-primary">
                ✅ เรียนจบแล้ว
              </button>
            )}
            {nextLesson ? (
